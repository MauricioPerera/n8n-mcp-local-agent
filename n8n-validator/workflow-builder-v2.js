...
function findBestTemplate(query, templates) {
    const q = query.toLowerCase();
    let best = null, bestScore = 0;
    for (const [key, tmpl] of Object.entries(templates)) {
        const keywords = tmpl.keywords || tmpl.description.toLowerCase().split(/\s+/).filter(w => w.length > 2);
        const qWords = q.split(/\s+/).filter(w => w.length > 2);
        let matches = 0;
        for (const qw of qWords) {
            for (const kw of keywords) {
                if (kw.includes(qw) || qw.includes(kw)) { matches++; break; }
            }
        }
        const score = (matches / Math.max(1, keywords.length)) * 100;
...
function extractSlotValues(query, slots) {
    const values = {};
    for (const slot of slots) {
        if (slot === "channel") {
            const m = query.match(/#([a-z0-9_-]+)/i);
            if (m) values[slot] = "#" + m[1];
        } else if (slot === "text") {
            const m = query.match(/(?:sends?|with|saying|message)\s+["']?([^"']+?)["']?\s*(?:to|via|in|$)/i);
            if (m) values[slot] = m[1].trim();
        } else if (slot === "url") {
            const m = query.match(/https?:\/\/[^\s]+/i);
            if (m) values[slot] = m[0];
        } else if (slot === "to") {
            const m = query.match(/(?:to|for)\s+([a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,})/i);
            if (m) values[slot] = m[1];
        } else if (slot === "subject") {
            const m = query.match(/(?:subject|about)\s+["']?([^"']+?)["']?\s*(?:$|to|for)/i);
            if (m) values[slot] = m[1].trim();
        }
    }
    return values;
...
async function main() {
    ...
    console.log("[2] Matching query: " + QUERY);
    const template = findBestTemplate(QUERY, templates);
    ...
    console.log("[3] Local validation...");
    const validation = validateLocal(code);
    ...
    console.log("[4] Deploying via MCP...");
